...
namespace Parseidon.Parser
{
    ...
    public class ParseidonParser
    {
        ...
        {
            Int32 oldPosition = state.Position;
            ASTNode actualNode = new ASTNode(46, "CSharpIdentCont", "", state.Position);
            Boolean result =
        ...
        private Boolean CheckRule_TMIdentifier(ASTNode parentNode, ParserState state, String? errorName)
        {
            ...
            Boolean result =
                SetErrorName(actualNode, state, "TextMate-Identifier",
                    (actualNode, errorName) => MakeTerminal(actualNode, state, errorName, false,
                        (actualNode, errorName) => CheckAnd(actualNode, state, errorName,
                            (actualNode, errorName) => CheckRule_TMIdentifierPrefix(actualNode, state, errorName),
                            (actualNode, errorName) => CheckAnd(actualNode, state, errorName,
                                (actualNode, errorName) => CheckRule_IdentStart(actualNode, state, errorName),
        ...
        private Boolean CheckRule_TMIdentifierPrefix(ASTNode parentNode, ParserState state, String? errorName)
        {
            Int32 oldPosition = state.Position;
            ASTNode actualNode = new ASTNode(55, "TMIdentifierPrefix", "", state.Position);
            Boolean result =
